...
const PasswordReset = () => {
  ...
  const validateToken = async () => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL || 'https://dental-admin-3.preview.emergentagent.com'}/api/auth/validate-reset-token/${token}`,
        {
          method: 'GET',
        }
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL || 'https://dental-admin-3.preview.emergentagent.com'}/api/auth/reset-password`,
        {
          method: 'POST',
          headers: {
